class Mover {
  ...
  applyMousePulse(pulseMag){
    let mouse = createVector(mouseX, mouseY)
    let dist = mouse.dist(this.position)
    let pulse = p5.Vector.sub(this.position, mouse)
    pulse.normalize()
    pulse.mult(pulseMag/dist)
    this.applyForce(pulse)
  }
...
function draw() {
  ...
  if(mouseIsPressed === true){
    let pulseMag = 50;
    moverA.applyMousePulse(pulseMag);
    moverB.applyMousePulse(pulseMag);
  }
